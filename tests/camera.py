"""Create a generic camera (go2rtc still image + RTSP) in the test instance via the config flow."""
...
HOST = os.environ.get("INTERCOM_TEST_GO2RTC", "127.0.0.1")  # go2rtc host for the test camera
...
def main() -> int:
    ...
    print("step:", status, flow.get("step_id"), [f.get("name") for f in flow.get("data_schema", [])])
    fid = flow["flow_id"]
    status, flow, _ = call(
        "POST",
    ...
    print("response:", status, flow.get("type"), flow.get("step_id"), flow.get("errors"))
    steps = 0
    while flow.get("type") == "form" and steps < 3:
        ...
        print("next:", status, flow.get("type"), flow.get("step_id"), flow.get("errors"))
    if flow.get("type") == "create_entry":
        print("camera created:", flow["result"].get("title"))
        return 0
    print("camera not created:", flow)
    return 1
